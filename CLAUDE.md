...
- **Pet license formats (2026-09-23):** a pet order is a `skin` (card skin sticker, 1/2-pack $9.99/$15.99), a `card` (standalone license card printed on the Silhouette sheets and stuck on a PVC blank, 1/2-pack $8.95/$13.95) or a `bundle` (one of each, $15.99). Stored in `pet_orders.variant` (empty on older rows = skin). Prices + labels: `plcItem()` in `functions/_shared/pricing.js`, mirrored by `PLF_ITEM` in `public/pricing.js`. The homepage builder picks it on step 3 ("Choose your license"), skips the chip step for card-only, offers "Make it a bundle" on review, and takes `?format=card|bundle`; the retro game + mobile game pick it on the checkout station. Command Station's License sheets take pet card orders once their licence has been rendered with Generate Card in the order modal (the card is the top 1322×834 of that render); a card 2-pack takes two slots.
...
- **Goofy - **Goofy Licenses (GOAT line):** Builder lives at `public/goofy/index.html`. Canonical public URL is `https://goofylicenses.com/goat/` — `public/_redirects` rewrites `/goat/*` → `/goofy/*` (200, URL stays); the singular `goofylicense.com` → plural 301 lives in `functions/_middleware.js`. All image/asset refs in the builder are root-absolute (`/goofy/images/…`) so the rewrite works. QR destinations are configured in `public/goofy/qr-config.js` (`window.GOOFY_QR`: origin, `/line` path, `?src=` trackers) — never hardcode QR URLs elsewhere. Nominee photos come in by file picker, paste (a "Paste image" button using the async Clipboard API, or ⌘V/Ctrl+V anywhere on the nominees step, which goes to the active nominee) or dropping an image file on a nominee card; the lineup offers just two options, Standard and Custom (`data-variant="custom"`, resolved by `resolveVariant` to `custom-giver`, or to `custom-anon` if they already switched this visit; `?variant=custom|custom-giver|custom-anon|standard` deep links all work), and credit vs anonymous is the Credit me / Stay anonymous switch in the nominees step that flips `custom-giver` ↔ `custom-anon` in place (same price, same steps). **Shipping:** stamp mail is included in the G.O.A.T. processing fee (`GOOFY_PRICES.stamp = 0`; checkout offers "Stamp Shipping (included, no tracking)" at $0 plus Standard / Priority as tracked paid upgrades; the webhook maps a $0 shipping charge to `stamp`). Pet orders still pay $0.95 stamp. **Background scene performance:** the three mountain ridges are separate `<svg class="ridges layer lN">` elements (each its own compositor layer). The scroll parallax is CSS scroll-driven (`@supports (animation-timeline: scroll())`, keyframes `px-*`) with the old JS `parallax()` as the fallback. Don't merge the ridges back into one SVG, animate SVG children, or add `filter` to the clouds: that re-rasterises a full-width image every scroll frame (it made iPhone scrolling choppy). Text inputs are ≥16px so iOS doesn't zoom on focus, and the product carousel swipes (`swipeCarousel`, `touch-action: pan-y`). **Back from Stripe:** right before the redirect the builder saves everything (option, nominees, names, photos + crops, giver/anonymous, self-nominate, address) to IndexedDB (`goofy-builder` / `drafts` / `checkout`); Stripe's cancel link carries `?resume=1` and the browser Back button reloads the no-store page as a back_forward navigation, and either restores the draft at the review step (skipping the intro sting and QR ceremony). `goofy/success.html` deletes the draft; drafts older than 24h are ignored. We deliberately don't fetch Instagram profile photos (against Instagram's terms, unreliable, 320px max); the upload box suggests screenshotting one instead. `?src=` landings show the Right-of-Nomination modal, then the standard honor → recipient+address → $4.95 checkout flow. `src` is captured by `plf-track.js` into `plf_attr` and logged on the order. Print-ready QR PNGs are generated at `public/goofy/qr.html` (per-batch nomination URLs + the fixed licence URL). Every printed licence card gets its own QR: `QR.licenceUrl(orderId)` = `HTTPS://GOOFYLICENSES.COM/L/<ORDER-ID>`, all caps on purpose so it encodes in QR alphanumeric mode (29×29 grid instead of 37×37, ~35% bigger printed modules); `functions/_middleware.js` 302s `/l/<id>` (any case, any host) to `/goat?src=license-qr&o=<id>`, where the landing greets that recipient by name via `/api/goofy-recipient`. Custom licence art: `bg-custom-*.png` are QR-less blank templates; `product-*.png` carry baked QRs for display. **All card geometry lives in `public/goofy/card-render.js` (`GoofyCard.geometry`)**: the rounded photo window that covers the template's photo frame, the QR box (x851 y33 128×128, covers the standard card's printed QR), and the name baselines/max sizes on the signature rules. The builder's DOM preview places names with `GoofyCard.nameLayout` (same size, ink-centred x and baseline as the print), its checkout snapshot is `GoofyCard.render`, and Command Station re-prints with `GoofyCard.render` + the order id, so preview, success-page snapshot and print can't drift. Change geometry only there. The 6×4 Council certificate print page is `public/goofy/certificate.html` (URL-param fed: `?name=` `?variant=` `?o=` `?src=`, includes the viral-loop QR = `nominationUrl(batch)` + `&o=`; also `?date=YYYY-MM-DD` (defaults to today, "Given under seal this Nth day of…"), `?thermal=1`, `?qr=0`). Printout is strictly #000 on white: no gold, grays, gradients or emoji, because thermal label printers threshold light tones to white (the first print lost the gold header, the gray filing line and the sig rule). The seal is a vector SVG (`SEAL_ART`: Gerald the goat under a crown, matching the licence seal). Thermal toggle only adds heavier strokes and weights, and it is remembered in localStorage (Hide QR deliberately is not). The PNG export traces the live DOM layout (glyph positions, rules, seal, QR modules) at 300dpi, so it always matches the print; don't hand-maintain a separate canvas layout.

- **Goofy Licenses house brand (umbrella, 2026-09-18):** goofylicenses.com is the house brand; each product is a "line" listed in `public/lines.js` (client: copy, prices, links, colors, `live`/`soon`) mirrored by `functions/_shared/lines.js` (server: ids, status, per-line email sender name). Lines today: `plc` (Pet Driver's License, live), `goat` (G.O.A.T., live), `forklift` + `clown` (soon, waitlist only). Adding a line = an entry in both registries + its builder + its email templates; the homepage tiles, coming-soon page, Command Station line switcher and Mac widget pick it up. **Routing** is `functions/_middleware.js` (Pages ignores host-based `_redirects` rules, so the old singular-domain rule never worked): goofylicenses.com `/` serves `public/goofy-home/`, `/pet` serves the PLF homepage with URL kept (canonical Link header → petlicensefactory.com), `/forklift` `/clown` serve `goofy-home/soon.html`, and every other PLF page reached via the Goofy domain gets a canonical header pointing at petlicensefactory.com; `www.goofylicenses.com`, `goofylicense.com`, `www.goofylicense.com` 301 to goofylicenses.com; G.O.A.T. pages (`/goat*`, `/goofy/*` HTML, not assets) opened on petlicensefactory.com 301 to goofylicenses.com; `/api/*` is never redirected. petlicensefactory.com stays the default for PLF (ads, TikTok Shop, creator + gift links, emails). **Orders:** `pet_orders.brand` holds the line id (`plc` | `goat`; legacy `goofy` = goat; always read it via `lineOfBrand` / `lineOfOrder`). admin-api `list_orders`/`get_order` return `line` plus the Goofy columns (probed via information_schema because those columns are added lazily on the first Goofy order); ship/fulfil/resend emails are line-aware (goat orders get `sendGoofyShippedEmail` / `sendGoofyConfirmationEmail`, never a PLF email). **Goofy email sender:** `LINE_SENDER.goat` = "The Council of G.O.A.T. Affairs" <hello@goofylicenses.com> (from + reply-to). goofylicenses.com is SendGrid domain-authenticated (CNAMEs `em5789`, `s1._domainkey`, `s2._domainkey` → `*.u28032385.wl136.sendgrid.net`, DNS only, plus `_dmarc` TXT `v=DMARC1; p=none;` in Cloudflare DNS), and Cloudflare Email Routing forwards hello@goofylicenses.com → contact@creditcardart.com. **GOAT checkout:** Stripe `branding_settings.display_name` = "Goofy Licenses" (+ house colors) and `statement_descriptor_suffix` "GOAT" (create-checkout-session.js); the Stripe account itself stays the PLF business. **Card re-print:** `public/goofy/card-render.js` (`GoofyCard.render`) redraws a GOAT card from the order with the shared card geometry + that card's own QR (`QR.licenceUrl(orderId)`); Command Station's GOAT panel shows it with a "License card PNG" download. The builder sends each nominee's photo crop, stored as `pet_orders.photo_crop` (JSON `{cx,cy,zoom}`, lazy column) so re-prints frame the photo as the customer did. **GOAT print docs:** Command Station's print preview gives GOAT orders the Council envelope sticker (`renderGoatEnvelopeLabel`, 6×4, pure black for the thermal printer: "Deliver to the Honorable" + nominee, a "Nominated by" line that never reveals an anonymous giver, filing no., seal). Gerald's seal art lives in `public/goofy/seal.js` (`GOOFY_SEAL`), shared by the certificate and the envelope; edit it there. **Command Station look (2026-09-18):** the approved "Command Station, all lines" mockup: house theme tokens at the top of its stylesheet (paper/navy, Bricolage Grotesque + IBM Plex, navy sidebar in both themes, winking-license mark), sidebar Lines → Workspace → Tools → General; stat cards To ship / Paid orders 7d / Revenue 7d / Waitlist signups (split per line); grouped tabs `STATUS_TABS` To ship (paid) / Shipped (processed+shipped+complete) / Unpaid (pending) / All, opening on To ship when anything is waiting; table Line / Order / For / Customer / Status / Total / Age; and at ≥1440px a docked order panel (`renderOrderPane`: card or photo, key fields, Open full order / certificate / card PNG / Print docs, email states). A row click selects into the panel; a double-click or "Open full order" opens the existing order modal (every tool still lives there). Below 1440px a row click opens the modal. The theme block at the end of the stylesheet keeps layout rules in min-width queries so the phone layout still applies. **Waitlists:** `/api/waitlist` → `plf_waitlist` (lazy table; dedup per line+email; honeypot; 10/hour per hashed IP), read by admin-api `list_waitlist`.
