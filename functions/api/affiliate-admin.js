...
import { sendCreatorOnboardingEmail } from '../_shared/email.js';
  ...
  try {
    switch (action) {
      ...
      case 'invite_creator': {
        ...
        const codeShape             = validateCodeShape(body.couponCode);
        ...
        if (dupEmail.rows.length > 0) {
          return json(400, { error: `${email} is already a creator.` });
        }

        const codeCheck = await ensureCodeUnique(env, db, codeShape.code);
        if (!codeCheck.ok) return json(400, { error: codeCheck.error });

        // Reserve the row first with status=pending so partial failures
        // are visible in the list view (not invisible).
        ...
        // Create Stripe coupons + promo codes
        let coupons;
        try {
          ...
          return json(500, {
            error:       `Coupon creation failed: ${err.message || err}`,
        ...
        // Send onboarding (logged either way)
        let emailResult = null;
        if (!skipEmail) {
          emailResult = await sendOnboardingAndLog(env, db, {
            creatorId, creatorName: name, creatorEmail: email,
...
function clampRate(v, fallback) {
  ...
  // Accept either 0–1 (0.2) or 0–100 (20). Anything ≥ 1.5 we treat as percent.
...
async function sendOnboardingAndLog(env, db, opts) {
